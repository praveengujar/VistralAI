  ...
  const routeGroups = [
    {
      ...
      bgColor: '#1E3A5F',
      routes: [
        { method: 'ALL', path: '/api/auth/[...nextauth]', desc: 'NextAuth handler (credentials, OAuth)' },
    ...
    },
    {
      ...
      routes: [
        ...
        { method: 'POST', path: '/api/aeo/insights/[insightId]/dismiss', desc: 'Dismiss with reason' },
        ...
        { method: 'POST', path: '/api/aeo/corrections/[workflowId]/approve', desc: 'Approve fix' },
        { method: 'POST', path: '/api/aeo/corrections/[workflowId]/verify', desc: 'Verify post-fix improvement' },
        ...
        { method: 'GET', path: '/api/aeo/reports/[reportId]', desc: 'Specific report' },
    ...
    },
    {
      ...
      routes: [
        ...
        { method: 'POST', path: '/api/onboarding/confirm', desc: 'Confirm extracted brand data' },
        { method: 'GET', path: '/api/onboarding/review-queue', desc: 'Items needing human review' },
        { method: 'POST', path: '/api/onboarding/review-queue/approve', desc: 'Approve/reject items' },
        { method: 'POST', path: '/api/onboarding/products/upload', desc: 'Bulk product upload' }
      ]
    },
    {
      id: 'admin',
      ...
      bgColor: '#7F1D1D',
      routes: [
        { method: 'GET', path: '/api/admin/queue-stats', desc: 'Job queue statistics' },
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '16px' }}>
        <div style={{ width: '2px', height: '24px', background: 'linear-gradient(to bottom, #6366F1, #475569)' }} />
      </div>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <span style={{ color: 'white', fontWeight: '700', fontSize: '14px', letterSpacing: '1px' }}>
              MIDDLEWARE LAYER
            </span>
          </div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px' }}>
            {middlewareStack.map((mw) => (
              <div key={mw.name} style={{
                background: 'rgba(255,255,255,0.15)',
                borderRadius: '8px',
                padding: '6px 12px',
                backdropFilter: 'blur(4px)'
              }}>
                <code style={{ color: 'white', fontSize: '11px', fontWeight: '500' }}>{mw.name}</code>
              </div>
            ))}
          </div>
        </div>
          ...
          >
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                ...
                <span style={{ color: group.color, fontWeight: '700', fontSize: '13px', letterSpacing: '1px' }}>
                  {group.name}
                </span>
                <span style={{
                  color: '#94A3B8',
              ...
              <svg
                width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={group.color} strokeWidth="2"
                style={{ transform: expandedGroups[group.id] ? 'rotate(180deg)' : 'rotate(0deg)', transition: 'transform 0.2s' }}
              >
                ...
                {group.routes.map((route, idx) => (
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                      <span style={{
                        background: getMethodColor(route.method),
                        color: 'white',
                        fontSize: '9px',
                        fontWeight: '700',
                        padding: '2px 6px',
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px' }}>
            {['5 min TTL default', 'Cache on read-heavy endpoints', 'Invalidate on mutations', 'In-memory fallback'].map((item) => (
              <div key={item} style={{
                background: 'rgba(255,255,255,0.15)',
                borderRadius: '8px',
                padding: '6px 12px',
                ...
                <span style={{ color: 'white', fontSize: '11px' }}>{item}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{
        ...
        {routeGroups.map((group) => (
          <div key={group.id} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <div style={{ width: '16px', height: '16px', borderRadius: '4px', background: group.color }} />
