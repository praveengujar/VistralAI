...
export async function addCrawlJobDone(
  ...
  await redisEvictConnection.expire("crawl:" + id + ":jobs_done", 24 * 60 * 60);
...
// === Cross-Crawl Cache Functions ===

/**
 * Generates a cache key for a URL in the global cache
 */
function generateGlobalCacheKey(url: string): string {
  // Use SHA256 hash to create a consistent key for the URL
  const hash = createHash("sha256").update(url).digest("hex");
  return `global:crawl_cache:${hash}`;
}

/**
 * Gets cached scrape result from the global cache
 * Returns null if not found or if cross-crawl cache is disabled
 */
export async function getGlobalCachedScrape(
  url: string,
  __logger: Logger = _logger,
): Promise<any | null> {
  if (!config.ENABLE_CROSS_CRAWL_CACHE) {
    return null;
  }

  return await withSpan("firecrawl-redis-get-global-cache", async span => {
    const cacheKey = generateGlobalCacheKey(url);
    setSpanAttributes(span, {
      "cache.key": cacheKey,
      "cache.url": url,
      operation: "get_global_cache",
    });

    try {
      const cached = await redisEvictConnection.get(cacheKey);

      if (cached) {
        __logger.debug("Global cache hit for URL", {
          url,
          cacheKey,
          module: "crawl-redis",
          method: "getGlobalCachedScrape",
        });
        setSpanAttributes(span, { "cache.hit": true });

        // Increment cache hit counter
        await redisEvictConnection.incr("global:crawl_cache:stats:hits").catch(() => {});

        return JSON.parse(cached);
      }

      setSpanAttributes(span, { "cache.hit": false });

      // Increment cache miss counter
      await redisEvictConnection.incr("global:crawl_cache:stats:misses").catch(() => {});

      return null;
    } catch (error) {
      __logger.warn("Error getting global cache", {
        error,
        url,
        module: "crawl-redis",
        method: "getGlobalCachedScrape",
      });
      setSpanAttributes(span, { "cache.error": true });
      return null;
    }
  });
}
...
/**
 * Sets cached scrape result in the global cache
 * Only caches if cross-crawl cache is enabled
 */
export async function setGlobalCachedScrape(
  url: string,
  data: any,
  ttlMs?: number,
  __logger: Logger = _logger,
  ...
  return await withSpan("firecrawl-redis-set-global-cache", async span => {
    const cacheKey = generateGlobalCacheKey(url);
    const ttl = ttlMs ?? config.CRAWL_CACHE_TTL_MS;
    ...
    try {
      ...
      // Increment cache store counter
      await redisEvictConnection.incr("global:crawl_cache:stats:stores").catch(() => {});

      __logger.debug("Cached scrape result globally", {
        url,
        cacheKey,
        ttlSeconds,
        module: "crawl-redis",
        method: "setGlobalCachedScrape",
      });
      setSpanAttributes(span, { "cache.stored": true });
    } catch (error) {
      __logger.warn("Error setting global cache", {
        error,
        url,
        module: "crawl-redis",
        method: "setGlobalCachedScrape",
