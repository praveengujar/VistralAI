...
import {
  ...
  saveCrawl,
  StoredCrawl,
  getGlobalCachedScrape,
  setGlobalCachedScrape,
...
async function processJob(job: NuQJob<ScrapeJobSingleUrls>) {
  ...
  try {
    ...
    // Check global cache for this URL
    logger.debug("Checking global cache for URL", {
      url: job.data.url,
      cacheEnabled: config.ENABLE_CROSS_CRAWL_CACHE,
      module: "queue-worker",
      method: "processJob",
    });
    const cachedResult = await getGlobalCachedScrape(job.data.url, logger);
    let pipeline: ScrapeUrlResponse | null = null;
    let timeoutHandle: NodeJS.Timeout | null = null;

    if (cachedResult) {
      // Use cached result instead of scraping
      logger.info("✅ Cache HIT - Using cached scrape result", {
        url: job.data.url,
        module: "queue-worker",
        method: "processJob",
      });
      pipeline = cachedResult;
    } else {
      logger.debug("Cache MISS - Proceeding with fresh scrape", {
        url: job.data.url,
        cacheEnabled: config.ENABLE_CROSS_CRAWL_CACHE,
        module: "queue-worker",
        method: "processJob",
    ...
    // Store result in global cache if this was not a cache hit
    if (!cachedResult) {
      logger.debug("Storing scrape result in global cache", {
        url: job.data.url,
        cacheEnabled: config.ENABLE_CROSS_CRAWL_CACHE,
        module: "queue-worker",
        method: "processJob",
      });
      await setGlobalCachedScrape(job.data.url, pipeline, undefined, logger);
    }
